package shipDock.framework.core.manager
{
	...
	public class SingletonManager implements ISingleton
	{
		...
		{
			(instance == null) && (instance = new SingletonManager());
			return instance;
		}
		...
		{
			(instance != null) && throwSingletonError(this); //检查是否被创建多次
		...
		private function findSingleton(target:*, isRemove:Boolean = false):ISingleton
		{
			...
			{
				var fullName:String;
				if ((target is String) && (String(target).indexOf("::") != -1))
				{ //若使用完全限定名的方式
		...
		public function hasSingleton(instance:ISingleton, isThrowError:Boolean = true):Boolean
		{
			...
			(result && isThrowError) && SingletonManager.throwSingletonError(instance);
			return result;
		}
