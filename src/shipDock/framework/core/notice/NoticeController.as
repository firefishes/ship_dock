	...
	final public class NoticeController extends SingletonBase
	{
		...
		public function sendNotice(notice:INotice):*
		{
			...
			while (i < max)
			{
				...
				if (!!element)
				{
					if (!!actionName)//用于命令机制的消息
					{
						notice.actionName = actionName;
						(!!notice.subCommand) && (args = [notice, actionName]); 
					}
					(!args) && (args = [notice]); //普通消息机制
					if (notice.observer && (notice.observer != observer))
					{ //判断是否只在特定发送者中触发消息侦听器
						i++;
						continue;
					}
