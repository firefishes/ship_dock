...
package starling.extensions
{
    ...
    public class PDParticleSystem extends ParticleSystem
    {
		...
		public static var configMannager:Object = new Object();//例子特效配置管理器
		public var pex:String;
        public function PDParticleSystem(config:XML, texture:Texture,pex:String)
        {
			this.pex = pex;
            parseConfig(config,pex);
        ...
        {
			return ParticlePool.instance.particle;
        ...
        private function parseConfig(config:XML,pex:String):void
        {
			if (configMannager[pex])//已经加载过的配置
			{
				var pexConfig:Object = configMannager[pex];
				mEmitterXVariance = pexConfig.mEmitterXVariance;
				mEmitterYVariance = pexConfig.mEmitterYVariance;
				mGravityX = pexConfig.mGravityX;
				mGravityY = pexConfig.mGravityY;
				mEmitterType = pexConfig.mEmitterType;
				mMaxNumParticles = pexConfig.mMaxNumParticles;
				mLifespan = pexConfig.mLifespan;
				mLifespanVariance = pexConfig.mLifespanVariance;
				mStartSize = pexConfig.mStartSize;
				mStartSizeVariance = pexConfig.mStartSizeVariance;
				mEndSize = pexConfig.mEndSize;
				mEndSizeVariance = pexConfig.mEndSizeVariance;
				mEmitAngle = pexConfig.mEmitAngle;
				mEmitAngleVariance = pexConfig.mEmitAngleVariance;
				mStartRotation = pexConfig.mStartRotation;
				mStartRotationVariance = pexConfig.mStartRotationVariance;
				mEndRotation = pexConfig.mEndRotation;
				mEndRotationVariance = pexConfig.mEndRotationVariance;
				mSpeed = pexConfig.mSpeed;
				mSpeedVariance = pexConfig.mSpeedVariance;
				mRadialAcceleration = pexConfig.mRadialAcceleration;
				mRadialAccelerationVariance = pexConfig.mRadialAccelerationVariance;
				mTangentialAcceleration = pexConfig.mTangentialAcceleration;
				mTangentialAccelerationVariance = pexConfig.mTangentialAccelerationVariance;
				mMaxRadius = pexConfig.mMaxRadius;
				mMaxRadiusVariance = pexConfig.mMaxRadiusVariance;
				mMinRadius = pexConfig.mMinRadius;
				mRotatePerSecond = pexConfig.mRotatePerSecond;
				mRotatePerSecondVariance = pexConfig.mRotatePerSecondVariance;
				mStartColor = pexConfig.mStartColor;
				mStartColorVariance = pexConfig.mStartColorVariance;
				mEndColor = pexConfig.mEndColor;
				mEndColorVariance = pexConfig.mEndColorVariance;
				mBlendFactorSource = pexConfig.mBlendFactorSource;
				mBlendFactorDestination = pexConfig.mBlendFactorDestination;
				return;
			}
			var obj:Object = new Object();
            mEmitterXVariance = parseFloat(config.sourcePositionVariance.attribute("x"));
			obj['mEmitterXVariance'] = mEmitterXVariance;
			
            mEmitterYVariance = parseFloat(config.sourcePositionVariance.attribute("y"));
			obj['mEmitterYVariance'] = mEmitterYVariance;

            mGravityX = parseFloat(config.gravity.attribute("x"));
			obj['mGravityX'] = mGravityX;

            mGravityY = parseFloat(config.gravity.attribute("y"));
			obj['mGravityY'] = mGravityY;

            mEmitterType = getIntValue(config.emitterType);
			obj['mEmitterType'] = mEmitterType;

            mMaxNumParticles = getIntValue(config.maxParticles);
			obj['mMaxNumParticles'] = mMaxNumParticles;

            mLifespan = Math.max(0.01, getFloatValue(config.particleLifeSpan));
			obj['mLifespan'] = mLifespan;
			
            mLifespanVariance = getFloatValue(config.particleLifespanVariance);
			obj['mLifespanVariance'] = mLifespanVariance;

            mStartSize = getFloatValue(config.startParticleSize);
			obj['mStartSize'] = mStartSize;

            mStartSizeVariance = getFloatValue(config.startParticleSizeVariance);
			obj['mStartSizeVariance'] = mStartSizeVariance;

            mEndSize = getFloatValue(config.finishParticleSize);
			obj['mEndSize'] = mEndSize;

            mEndSizeVariance = getFloatValue(config.FinishParticleSizeVariance);
			obj['mEndSizeVariance'] = mEndSizeVariance;

            mEmitAngle = deg2rad(getFloatValue(config.angle));
			obj['mEmitAngle'] = mEmitAngle;

            mEmitAngleVariance = deg2rad(getFloatValue(config.angleVariance));
			obj['mEmitAngleVariance'] = mEmitAngleVariance;

            mStartRotation = deg2rad(getFloatValue(config.rotationStart));
			obj['mStartRotation'] = mStartRotation;

            mStartRotationVariance = deg2rad(getFloatValue(config.rotationStartVariance));
			obj['mStartRotationVariance'] = mStartRotationVariance;

            mEndRotation = deg2rad(getFloatValue(config.rotationEnd));
			obj['mEndRotation'] = mEndRotation;

            mEndRotationVariance = deg2rad(getFloatValue(config.rotationEndVariance));
			obj['mEndRotationVariance'] = mEndRotationVariance;

            mSpeed = getFloatValue(config.speed);
			obj['mSpeed'] = mSpeed;

            mSpeedVariance = getFloatValue(config.speedVariance);
			obj['mSpeedVariance'] = mSpeedVariance;

            mRadialAcceleration = getFloatValue(config.radialAcceleration);
			obj['mRadialAcceleration'] = mRadialAcceleration;

            mRadialAccelerationVariance = getFloatValue(config.radialAccelVariance);
			obj['mRadialAccelerationVariance'] = mRadialAccelerationVariance;

            mTangentialAcceleration = getFloatValue(config.tangentialAcceleration);
			obj['mTangentialAcceleration'] = mTangentialAcceleration;

            mTangentialAccelerationVariance = getFloatValue(config.tangentialAccelVariance);
			obj['mTangentialAccelerationVariance'] = mTangentialAccelerationVariance;

            mMaxRadius = getFloatValue(config.maxRadius);
			obj['mMaxRadius'] = mMaxRadius;

            mMaxRadiusVariance = getFloatValue(config.maxRadiusVariance);
			obj['mMaxRadiusVariance'] = mMaxRadiusVariance;

            mMinRadius = getFloatValue(config.minRadius);
			obj['mMinRadius'] = mMinRadius;

            mRotatePerSecond = deg2rad(getFloatValue(config.rotatePerSecond));
			obj['mRotatePerSecond'] = mRotatePerSecond;

            mRotatePerSecondVariance = deg2rad(getFloatValue(config.rotatePerSecondVariance));
			obj['mRotatePerSecondVariance'] = mRotatePerSecondVariance;

            mStartColor = getColor(config.startColor);
			obj['mStartColor'] = mStartColor;

            mStartColorVariance = getColor(config.startColorVariance);
			obj['mStartColorVariance'] = mStartColorVariance;

            mEndColor = getColor(config.finishColor);
			obj['mEndColor'] = mEndColor;

            mEndColorVariance = getColor(config.finishColorVariance);
			obj['mEndColorVariance'] = mEndColorVariance;

            mBlendFactorSource = getBlendFunc(config.blendFuncSource);
			obj['mBlendFactorSource'] = mBlendFactorSource;

            mBlendFactorDestination = getBlendFunc(config.blendFuncDestination);
            obj['mBlendFactorDestination'] = mBlendFactorDestination;
			
			configMannager[pex] = obj;
